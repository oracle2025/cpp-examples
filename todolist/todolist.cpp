...
{
	int line = g_list_view->value();
	if (!line) {
		return;
	}
	const char *result;
	result = fl_input("Edit Entry:", g_list_view->text(line));
	if (!result) {
		return;
	}
	if (std::string(result).empty()) {
...
void delete_cb(Fl_Widget*, void*)
{
	...
	if (fl_choice("Delete Entry: \n%s", "No, Cancel", "Yes, Delete!", 0, g_list_view->text(line))) {
